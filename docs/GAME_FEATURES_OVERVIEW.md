...
一个完整的游戏 = `state` + `rules` + `net_adapter` + `register.cpp` + `config/game.json` + `web/`

| 组件 | 职责 |
|------|------|
| **IGameState** | 游戏状态：当前玩家、是否终局、胜者、clone/hash、多步动作起点标志 |
| **IGameRules** | 合法动作、执行动作、撤销动作 |
| **IFeatureEncoder** | 状态 → 神经网络输入张量 + 合法掩码 |
| **game.json** | 训练超参数配置 |
| **Web 前端** | 玩家交互界面，验收训练结果 |
...
神经网络引导的 Monte Carlo Tree Search，原生支持 N 人游戏（2-4 人）。主要配置：`simulations`、`c_puct`、`temperature`（支持 schedule）、Dirichlet 噪声。

### 残局求解（Tail Solve）

MCTS 前用 alpha-beta 尝试精确求解，证明必赢则跳过 MCTS。使用 paranoid 假设（所有对手联合针对当前玩家），支持任意玩家数。仅在 proven win 时替代 MCTS（proven loss 在多人场景下过于悲观，对手之间有利益冲突时不一定成立）。内置 `AlphaBetaTailSolver`，通过 `depth_limit` + `node_budget` 控制预算。适用于分支因子小且 `legal_actions` 开销低的游戏。详见 [Guide §9.2](GAME_DEVELOPMENT_GUIDE.md#92-itailsolver--残局求解器)。
...
MCTS 遍历到随机转移时，两种处理方式：

| 方式 | 机制 | 适用 |
|------|------|------|
| **Chance node** | 在随机边界展开多个可能结果（采样，cap 限制分支数），继续向下搜索 | 随机事件频繁，需要跨随机边界的搜索深度 |
| **Truncate** | 遇随机转移时做一次采样 + NN eval，不继续展开 | 随机事件稀疏或分支因子大，展开不划算 |

随机采样的来源有两种：直接从状态构建（如 Azul 的袋中剩余可从公开状态推导出完整组成）或从 belief tracker 构建（如 Splendor 的对手暗牌不在我的可观察状态中）。通过注册 `stochastic_detector` 启用，`enable_chance_sampling` 控制 chance node vs truncate。确定性游戏不注册 detector。详见 [Guide §10](GAME_DEVELOPMENT_GUIDE.md#10-物理随机性)。

**维度二：信息不对称**（对手暗牌等）

当游戏存在玩家间的非对称隐藏信息时（如 Splendor 盲压暗牌——执行者知道是什么，对手不知道），需要额外机制：

- 每次 MCTS simulation 开头从 belief 采样对手隐藏状态（ISMCTS 方式）
- Encoder 保证信息屏障：对手视角看自己的采样牌，看我的牌是占位符
- Belief tracker 维护"我知道什么"，用于采样

通过注册 `belief_tracker` 启用。详见 [Guide §11](GAME_DEVELOPMENT_GUIDE.md#11-隐藏信息与-belief-tracker)。

**训练增强：Peek 模式**

Peek 是训练增强手段（类似启发式引导），用全知状态搜索（忽略随机性和隐藏信息），帮助训练早期先学稳策略。仅在训练 selfplay 中使用，实战 AI 和 arena/eval 不使用。通过 `game.json` 的 `peek_steps` 参数控制：前 `peek_steps` 个训练步用 peek，之后自动切换为 nopeek。默认 0（始终 nopeek）。详见 [Guide §9](GAME_DEVELOPMENT_GUIDE.md#9-训练可选特性)。

**现有游戏的组合**：

| 游戏 | 物理随机 | 信息不对称 | 方案 |
|------|---------|-----------|------|
| TicTacToe / Quoridor | 无 | 无 | 无需额外处理 |
| Azul | 跨轮发砖 → Truncate | 袋中剩余从公开信息推导 → tracker 维护 | detector + tracker |
| Splendor | 翻新卡 → Chance node | 盲压暗牌 → ISMCTS 根采样 | detector + tracker |
| Love Letter | 抽牌 → Chance node | 对手手牌 → ISMCTS 根采样 | detector + tracker |
| Coup | 抽牌 → Chance node | 对手暗牌 → ISMCTS 根采样 | detector + tracker |
...
这套 API 的首要用途**不是**生产部署，而是**证明 AI 决策链路不读真实状态**——CLAUDE.md 的核心设计原则要求 AI 只依赖观察序列，但靠 code review 根本无法保证（一个 `checked_cast` + 隐藏字段读取就能悄悄泄漏）。唯一能硬性证明分离的办法就是：把 AI 放到独立 session，用和 ground truth **不同的 seed** 初始化，只靠公共事件同步，然后检查两边的 belief 是否逐步等价。
...
用户走棋后触发异步 pipeline：分析用户动作质量 → AI 思考并落子 → 预计算下一步分析。分析基于 MCTS 根边的 per-player Q 值，一次搜索同时得到所有合法动作的胜率，计算用户实际选择与最优动作的掉分百分比。支持任意人数。详见 [Guide §14.3](GAME_DEVELOPMENT_GUIDE.md#143-ai-pipeline-与动作分析)。
...
| `belief_tracker` | 有隐藏信息 | 追踪对手可能持有的信息 |
| `stochastic_detector` | 有随机事件 | 标记哪些动作包含随机性 |
...
参考实现：TicTacToe（最简）、Quoridor（中等）、Splendor（完整，含随机/隐藏信息）、Love Letter（隐藏信息 + 玩家淘汰）、Coup（虚张声势 + 多阶段回合 + 玩家淘汰）
...
当前的 belief tracker 只追踪**确定性信息**——某张牌被打出过、某个角色已暴露、deck 剩余组成可从公开信息精确推导。`randomize_unseen` 在已知约束下均匀采样未知部分，没有利用历史动作序列中蕴含的概率信号。

下一步是引入**概率化 belief**：用时序模型（Transformer / RNN）处理观察历史，输出对手隐藏状态的概率分布，而非均匀采样。例如，对手在 Splendor 中跳过购买高分卡的机会，暗示其暗牌可能已经是同色高分卡；Love Letter 中对手不打 Guard 猜牌，暗示其手牌可能就是 Guard。这些信号目前被忽略。

**核心难点**在于概率 belief 必须同时正确嵌入两个已有机制：

- **IS-MCTS 根采样**：当前每次 simulation 从 belief 均匀采样一个可能世界。概率化后，采样需要按 belief 分布加权——高概率的对手状态被更频繁地搜索。这改变了 MCTS 的探索分布，需要验证搜索仍能收敛且不会过拟合到单一假设。
- **物理随机性（chance node / truncate）**：随机事件（翻卡、抽牌）的采样来源可能依赖 belief。例如 Splendor 翻新卡时，deck 的组成是 belief 的一部分；如果 belief 分布是非均匀的，chance node 的分支权重也应反映这一点。两者必须协调——不能 IS 采样用了概率 belief 而 chance 采样仍用均匀分布，否则搜索中的世界自洽性会被破坏。

实现路径可能是：训练一个独立的 belief network（输入为观察序列，输出为对手隐藏状态的分布），在 MCTS 根节点用它替代均匀采样，同时将同一分布传递给 stochastic detector 的采样逻辑。
