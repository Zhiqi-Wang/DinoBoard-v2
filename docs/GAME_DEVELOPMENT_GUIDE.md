...
对于有随机事件的游戏（翻牌、抽卡等），每次随机事件后返回不同的值。`default_stochastic_detector` 通过比较前后状态的 `rng_nonce()` 来检测随机转移。

确定性游戏（如 Quoridor、TicTacToe）不需要覆盖。
...
执行动作但**不解决随机性**。仅当启用残局求解（Tail Solver）时才需要实现——不用残局求解的游戏可以跳过，基类默认实现直接委托给 `do_action_fast`。

- 确定性游戏（如 Quoridor、TicTacToe）：默认实现已正确，无需额外代码
- 随机游戏且启用残局求解：必须显式实现。随机事件产生的结果用**占位符**替代，并且规则引擎必须正确处理占位符的语义：
  - 例如抽牌：抽到的是一张占位符牌，规则引擎需要标记这张牌不可使用（不能打出、不能作为资源）
  - 例如抽取被动 buff：改为一个无效果的占位 buff，不触发任何能力
  - 关键：占位符不能破坏游戏逻辑的完整性。残局求解器会在这个确定化状态上展开搜索树，所有后续的 `legal_actions`、`is_terminal`、胜负判定都必须能正确处理包含占位符的状态

### 3.3 随机游戏必须实现

#### `chance_outcomes(state, action) -> vector<ChanceOutcome>`

返回某个动作所有可能的随机结果及概率。例如 Splendor 中翻一张新卡时，每张牌库中的牌都是一个 outcome。回放系统和确定化搜索依赖此方法。
...
#### `do_action_with_outcome(state, action, outcome_id) -> UndoToken`

用指定的随机结果执行动作。用于回放和 MCTS 中的确定化搜索。

确定性游戏（无随机事件）不需要实现这两个方法。
...
| 6 | `belief_tracker` | `unique_ptr<IBeliefTracker>` | 否 | 隐藏信息游戏的信念追踪器 |
| 7 | `stochastic_detector` | `StochasticTransitionDetector` | 否 | 检测随机转移（默认比较 `rng_nonce()`） |
| 8 | `enable_chance_sampling` | `bool` | 否 | NoPeek 根确定化开关（默认 true） |
| 9 | `state_serializer` | `StateSerializer` | 否 | 状态序列化为 JSON（Web 前端需要） |
| 10 | `action_descriptor` | `ActionDescriptor` | 否 | 动作语义描述（Web 前端需要） |
| 11 | `heuristic_picker` | `HeuristicPicker` | 否 | 启发式策略（warm start + eval benchmark） |
...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `peek_steps` | int | 0 | 前 N 个训练步用 peek 模式（全知搜索），之后切回 nopeek。0 表示始终 nopeek |

Peek 模式禁用 NoPeek traversal limiter，MCTS 用全知状态搜索（能看穿牌堆和对手手牌）。适合训练早期让 value head 先学到基本策略结构，再切到 nopeek 学习在信息不完全下决策。仅影响 selfplay，arena/eval 始终使用 nopeek。
...
**仅在训练 selfplay 中使用，实战 AI 不使用。** 对局时 AI 必须通过正式的 chance node + ISMCTS 搜索。

**适用场景**：随机性或隐藏信息较复杂的游戏（如 Splendor），训练初期网络太弱、正式搜索噪声过大时，用 Peek 模式收集一批高质量样本快速建立基本策略。

**配置**：通过 `game.json` 的 `training.peek_steps` 控制。前 `peek_steps` 个训练步使用 peek（selfplay 时不创建 NoPeek traversal limiter，MCTS 用全知状态搜索），之后自动切回 nopeek。默认 0（始终 nopeek）。Arena 和 eval 始终使用 nopeek，不受此参数影响。
...
### 10.1 StochasticTransitionDetector

**用途**：检测两个连续状态之间是否发生了随机转移。MCTS 使用它在遍历过程中识别随机边界。

默认实现 `default_stochastic_detector` 比较 `rng_nonce()`。大多数游戏只需要在 State 中正确实现 `rng_nonce()` 即可——每次随机事件后递增 nonce。

### 10.2 Chance Node vs Truncate

MCTS 遍历到随机边界时，两种处理方式：

| 方式 | 机制 | 适用场景 |
|------|------|----------|
| **Chance node** | 在随机边界采样多个可能结果作为子节点（`chance_expand_cap` 限制分支数），继续向下搜索 | 随机事件频繁、需要跨随机边界的搜索深度（如 Splendor 翻新卡） |
| **Truncate** | 遇随机转移时做一次采样 + NN eval，不继续展开子树 | 随机事件稀疏或分支因子大、展开不划算（如 Azul 跨轮 factory refill） |

通过 GameBundle 中的 `enable_chance_sampling` 控制：`true`（默认）= chance node，`false` = truncate。

### 10.3 Chance Node 工作机制

当 MCTS 遍历检测到随机转移时：

1. 计算 `key = hash(parent_state, action)` 标识这个 chance node
2. 若 `chance_pool[key].size() < cap`：clone parent → 随机采样（re-roll 或 `randomize_unseen`）→ 执行 action → 存入 pool
3. 若 pool 已满：跳过创建
4. 从 pool 中随机选一个结果替换当前状态，继续向下遍历

每个 (parent, action) 的 chance node 最多积累 `chance_expand_cap` 个不同的采样世界。`on_ply_complete()` 清空 pool 释放内存。

**随机采样的来源**取决于游戏的信息结构：

- **状态可推导**（如 Azul、Love Letter）：所有随机所需的信息已在状态中（Azul: bag 组成 = 总砖 - 可见砖；Love Letter: 未见牌 = 16 张全牌 - 手牌 - 弃牌堆 - 明牌），直接 re-roll action 用不同 RNG 种子即可得到不同结果。注意 Love Letter 虽然也注册了 belief tracker（因为有非对称隐藏信息），但其 `randomize_unseen` 的 seen pool 完全从当前状态推导（弃牌堆是完整的打出记录，不会被清除），不需要增量追踪
- **需要增量追踪**（如 Splendor）：牌会离开公开区域（被买走后不再可见），仅从当前状态无法推导出历史上见过哪些牌，须通过 belief tracker 维护 `seen_cards` 集合，`randomize_unseen` 先补全隐藏信息再执行 action

### 10.4 连续随机转移

如果一个随机节点的子节点仍然是随机节点（如两个玩家连续买面朝上的卡），每一层各自触发 chance pool 分支。两层 cap=10 意味着最多 100 个组合，三层 1000 个。实践中需关注分支爆炸——如果连续随机转移频繁，考虑使用 truncate 或降低 cap。

### 10.5 开发者 Checklist

1. 在 State 中实现 `rng_nonce()`，每次随机事件后递增
2. 在 GameBundle 中注册 `stochastic_detector`（通常用 `default_stochastic_detector`）
3. 设置 `enable_chance_sampling`：频繁随机用 `true`，稀疏/高分支用 `false`
4. 设置 `chance_expand_cap`（默认 10）
...
**关键区分**：如果所有玩家对隐藏信息同等无知（如 Azul 的袋中剩余砖——没人知道顺序，但组成可从公开状态推导），这不算非对称隐藏信息，不需要 belief tracker。直接从状态构建随机采样即可（见 §10.3）。

### 11.1 架构原理

非对称隐藏信息需要在 MCTS 中额外处理——仅靠 chance node 不够，因为信息缺口不是"随机事件"而是持续存在的：

1. **ISMCTS 根采样**：每次 MCTS simulation 开头，从当前玩家的 belief 采样对手的隐藏状态，写入 state 对象。整条 simulation 在这个 belief-consistent 的世界中搜索
2. **Encoder 信息屏障**：即使 state 中已填入采样值，encoder 仍保证——对手视角看自己的采样牌（`is_self=true`），看我的牌是占位符（`is_self=false`）。这与训练时的 encoding 模式一致
3. **跨 simulation 共享树**：不同 simulation 采样不同的隐藏状态，但共享同一棵 MCTS 树。Visit count 自然是跨多种可能世界的加权平均

信息不对称处理与物理随机性处理**正交**——同一个游戏可以同时需要两者（如 Splendor：ISMCTS 根采样 + chance node 翻新卡）。
...
3 个必须实现的方法：
- `init(state, player)` — 记录初始可见信息，设置视角玩家
- `observe_action(before, action, after)` — 每步后更新已知信息
- `randomize_unseen(state, rng)` — 用 belief 替换 state 中的隐藏字段

**Belief tracker 不仅追踪公开信息，也可以追踪通过游戏技能合法获得的私有知识**。例如 Love Letter 中 Priest 偷看对手手牌、King 交换后知道对方原来的牌——这些是当前玩家通过正当途径获得的精确信息，应记录在 tracker 中。`randomize_unseen` 时优先使用 tracker 中的确定知识（直接固定），没有确定知识的才从 unseen pool 中随机采样。这使得 ISMCTS 的采样质量更高——已知的不浪费预算重新猜。

**重要约束**：`randomize_unseen()` 绝不能读取游戏状态的隐藏字段（deck 内容、对手暗牌 ID）。tracker 记录的私有知识必须完全来自 `observe_action()` 中的合法观察，而非直接读取 state。理由：引擎未来须支持 API 模式，由外部游戏服务器持有真实状态，AI 只接收观察序列。详见 CLAUDE.md "AI Pipeline Independence from Game State" 节。

参考 `games/splendor/splendor_net_adapter.h` 中的 `SplendorBeliefTracker` 实现。

### 11.3 Belief Tracker 生命周期

框架在所有代码路径（selfplay、arena、constrained eval、web GameSession）中自动管理以下生命周期，游戏开发者只需实现 `IBeliefTracker` 的三个方法：

```
每一步 ply:
  1. belief_tracker->init(state, current_player)    // 设置视角
  2. MCTS 搜索：
     - 每次 simulation 开头：randomize_unseen       // ISMCTS 根采样
     - 遍历中遇 stochastic boundary：chance node    // 物理随机处理
  3. do_action_fast(state, chosen)                   // 执行动作
  4. belief_tracker->observe_action(before, action, after) // 记录新信息
  5. limiter->on_ply_complete()                      // 清空 chance_pool
```

框架根据 bundle 中注册的 `belief_tracker` 和 `stochastic_detector` 自动构造 limiter，开发者不需要手动创建。
...
- `init()`：记录 `perspective_player`，首次初始化清空 `known_hand`
- `observe_action()`：追踪 Priest（偷看 → 记录对手手牌）、Baron 平局（互看 → 记录双方手牌）、King（交换 → 更新/转移知识）、Prince（重摸 → 清除知识）、淘汰（清除）、对手打出已知牌（清除）
...
- `init()`：从状态初始化 seen（tableau + visible reserved + 自己的 reserved）
- `observe_action()`：增量追踪新揭示的卡牌（BuyFaceup/ReserveFaceup 翻新卡、perspective_player 的 ReserveDeck）
...
- **belief tracker**：通过 `observe_action` 积累的私有知识（Priest 偷看的手牌、Baron 比较的结果等）
...
1. 确认游戏是否有非对称隐藏信息（玩家间知道的不一样）。对称无知（如 Azul 的 bag）不需要 tracker
2. 实现 `IBeliefTracker` 的三个方法，遵守"绝不读取隐藏字段"约束
3. Encoder 中对非自身玩家的隐藏信息输出占位符
4. 在 GameBundle 中注册 `belief_tracker`
...
Pipeline 是对局中 AI 决策和分析的核心调度机制。它协调三件事：**分析用户走棋质量**、**AI 思考并落子**、**预计算下一步分析**。高级操作（悔棋、提示）也依赖 pipeline 的状态和数据。

#### 完整流程（专家难度）

```
[空闲] precompute 在后台对当前局面跑 MCTS（5000 simulations）
  │
  ▼ 用户走棋
  │
[analyzing] 从 precompute 结果中读取用户所选动作的 Q 值，计算掉分
  │
  ▼
[ai_thinking] AI 搜索并落子
  │
  ▼
[done] 触发新一轮 precompute（为下一步用户走棋做准备）
```

非专家难度跳过 analyzing 阶段和 precompute，只做 AI 思考落子。

前端通过轮询 `GET /pipeline` 获取当前阶段（`analyzing` / `ai_thinking` / `done` / `error`）。pipeline 在后台线程执行 MCTS 搜索，GIL 已通过 `py::gil_scoped_release` 释放，不会阻塞前端请求。
...
| `test_hidden_info.py` | NoPeek、belief tracking、随机性、不偷看验证 | ~27 |
...
- [ ] 部署模型：将 `runs/<name>_001/models/model_best.onnx` 复制到 `games/<name>/model/model_best.onnx`（web 服务器也接受旧名 `best_model.onnx`）
...
**确定性游戏**（无 `belief_tracker`、无 `stochastic_detector`）：
...
**随机游戏**（有 `belief_tracker` + `stochastic_detector`）：
1. 上面确定性游戏的 4 步
...
事件负责在 AI 侧同步 ground truth 的公开事实（翻的新卡、抽到的公共牌、挑战揭露的身份等）。AI 内部 state 在 do_action_fast 跑出来的随机结果会被 event 覆盖，belief tracker 通过正常的 `observe_action` 读到正确的 state_after。
...
5. `belief_tracker.observe_action(state_before, action, state_after)` — state_after 是 post-event 之后的版本
