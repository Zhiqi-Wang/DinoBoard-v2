...
2. **Web 对战平台**：统一入口，用户选择游戏后对战 AI，共享用户系统
3. **调试前端**：开发时可视化人机对战，辅助验证规则和 AI

## 核心原则

- 游戏开发者只写：C++ 规则引擎 + 特征编码 + 一个 config JSON
...
│
├── games/                            # 各游戏实现（只写规则 + 特征编码）
│   ├── splendor/
│   │   ├── include/
│   │   │   ├── splendor_rules.h
│   │   │   ├── splendor_state.h
│   │   │   └── splendor_net_adapter.h
│   │   ├── src/
│   │   │   ├── splendor_rules.cpp
│   │   │   ├── splendor_state.cpp
│   │   │   ├── splendor_net_adapter.cpp
│   │   │   ├── splendor_cards.cpp          # 静态数据（卡池、贵族）
│   │   │   └── splendor_heuristic.cpp      # 可选：游戏特定 heuristic
│   │   ├── config/
│   │   │   ├── train.json                  # 训练配置
│   │   │   └── game.json                   # 游戏元数据（feature_dim, action_space, nopeek 参数等）
│   │   └── web/                            # 调试/对战前端资源
│   │       ├── app.js
│   │       ├── index.html
│   │       └── styles.css
...
│   ├── auth/                         # 用户认证
│   │   ├── models.py                 # 用户/Session 数据模型
│   │   ├── routes.py                 # 注册/登录/登出 API
│   │   ├── security.py               # 密码哈希、CSRF、限流
│   │   └── middleware.py             # 认证中间件
│   ├── game_service/                 # 对局管理
...
绝对不能在持有数据库连接/事务的情况下执行。正确做法：

1. 从 DB 读取状态 → 关闭连接/释放锁
2. 在内存中执行 MCTS / tail solve（纯计算，不碰 DB）
3. 计算完成后再开连接写入结果

pipeline worker 和 analysis worker 都必须遵守这个原则，
否则一个用户的 AI 在思考时会阻塞所有其他用户的读写请求。
...
### 阶段 5：Web 对战平台
- 从 mosaic-azul-web 迁移用户系统（auth、session、tier）
